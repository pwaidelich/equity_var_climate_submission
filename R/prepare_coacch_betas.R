...
df_coacch <- read_csv("data/COACCH_damagefunctions/COACCH.csv") %>%
  ...
  # subset to parameters for the IMAGE regions (identified by prefix 'I_') and/or World
  filter(region == "World" | str_detect(region, "^I\\_"))
...
# create new empty columns with the distribution parameters for the region-specific COACCH damage function
# NOTE: this implements damage function uncertainty in COACCH
df_coacch$no_slr_a_norm_mean <- NA
df_coacch$no_slr_a_norm_sd <- NA

# populate the empty columns by fitting the distribution to the published COACCH percentiles
for(jj in 1:nrow(df_coacch)) {
  
  # select the jj-th region
  region_jj <- df_coacch$region[jj]
  
  # print the region name to track progress
  print("Next region:")
  print(region_jj)
  
  # optionally, pause for a few seconds to make it easier to view the charts produced below
  Sys.sleep(3)
  
  # select the jj-th region in df_coacch and convert to LONG format (one row per percentile)
  df_distr_jj <- df_coacch %>% filter(region == region_jj) %>% pivot_longer(cols = starts_with("no_slr_a_q")) %>%
    
    # clean the percentile column and convert to a numeric
    mutate(perc = str_extract(name, "0\\_[:digit:]+$") %>% str_replace("_", ".") %>% as.numeric())
  
  
  # estimate distribution using the rriskDistributions package
  df_normsolved_jj <- get.norm.par(p = df_distr_jj$perc, q = df_distr_jj$value,
                                   fit.weights = c(rep(1, 5), 10^10, rep(1, 5)))
  
  # assign mean and sd to the jj-th region in df_coacch
  df_coacch$no_slr_a_norm_mean[jj] <- df_normsolved_jj["mean"]
  df_coacch$no_slr_a_norm_sd[jj] <- df_normsolved_jj["sd"]
...
df_out <- bind_rows(df_iso, df_impute) %>%
  ...
  left_join(df_coacch %>% select(coacchregion = "region",
                                 starts_with("no_slr_a"),
                                 starts_with("no_slr_b")), by = "coacchregion") %>%
...
crossing(temp = seq(coacch_baseline_temperature, 7, by = 0.1), 
         df_out %>% select(coacchregion, no_slr_b1, no_slr_b2, no_slr_a_q_0_025, no_slr_a_q_0_975, no_slr_a_norm_sd)) %>%
  ...
  # calculate 95% CI based on the imputed Gaussian
  mutate(gdploss_025 = (1 - 1.96*no_slr_a_norm_sd)*gdploss,
         gdploss_975 = (1 + 1.96*no_slr_a_norm_sd)*gdploss,
         gdploss_025_actual = no_slr_a_q_0_025*gdploss,
         gdploss_975_actual = no_slr_a_q_0_975*gdploss) %>%
  
  # # floor the upper end at 100% (since consumption has a subsistence minimum in META)
  mutate(gdploss_975 = if_else(gdploss_975 > 100, 100, gdploss_975),
         gdploss_975_actual = if_else(gdploss_975_actual > 100, 100, gdploss_975_actual)) %>%
  ...
  # add a line for the 0% value
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey") +
  
  # add a line for the GDP loss
  geom_line(aes(color = chart_label)) +
  
  # add a shaded area ribbon for the 95% CI
  geom_ribbon(aes(ymin = gdploss_025, ymax = gdploss_975, fill = chart_label), alpha = 0.2) +
  
  geom_line(aes(y = gdploss_025_actual), color = "lightgrey", linetype = "dotted") +
  geom_line(aes(y = gdploss_975_actual), color = "lightgrey", linetype = "dotted") +
