...
create_expectedpv_timeseries <- function(data = NULL, # data with the portfolio weights for each country
                                         ...
                                         div_gdp_growth_multiplier = 1 # a 1% GDP growth (real) implies a x% real growth in dividends
                                         ) {
  ...
  # NOTE: 'gdp_growth.csv' is created in a previous R script, see ReadMe
  ...
  df_out <- df %>%
    ...
    # create a new column called gdp_growth_factor which is 1 + gdp_growth (e.g., 1.05 for a 5% year-to-year growth rate) times the multiplier
    # NOTE: in the base year, dividends do not grow yet, so growth factor is 1
    mutate(gdp_growth_factor = if_else(year == base_year, 1, 1 + div_gdp_growth_multiplier * gdp_growth)) %>%
...
readRDS(file.path("data", "weights", "pvweights_market_byyear_2024_2100.rds")) %>%
  ...
  filter(ssp == "SSP2", portfolio == "msciacwi") %>%
