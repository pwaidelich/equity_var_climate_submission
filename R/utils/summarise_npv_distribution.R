...
summarise_npv_distribution <- function(data = NULL) {
  ...
  data %>%
    ...
    select(rcp, ssp, portfolio, persist, amoc, iso3, mc_samplesize, saf_deactivated, tip, tdamage, base_year, end_year, starts_with("lossfactor_pv")) %>%
    ...
    group_by(rcp, ssp, portfolio, persist, amoc, saf_deactivated, mc_samplesize, tip, tdamage, base_year, end_year, discountrate, iso3) %>%
