...
# set label for spec with tipping points
label_tip_all <- "Incl. climate tipping points"
label_tip_none <- "Excl. climate tipping points"
...
# write a function to quickly load the lossfactors
get_lossfactors <- function(mc_samplesize = 10000, # number of MC runs used to calculate the loss distribution
                            ...
                            tdamage_selected = "coacch_distribution", # GDP-temperature damage function selected
                            ...
                            saf_deactivated = FALSE, # if the surface albedo feedback is deactivated 
                            iso3_subset = NULL,
                            dir_meta_mc_outputs = "data/META MC results" # directory where input lossfactors from META runs are stored)
) {
  ...
  if(saf_deactivated) identifier_string <- paste0(identifier_string, "_safnone")
  
  # ensure that we have a subfolder for this identifier_string
  if(!dir.exists(file.path(dir_meta_mc_outputs, identifier_string))) stop(paste0("No META MC outputs exist for the identifier string ", identifier_string))
  
  # map all expected lossfactor CSV files
  filepaths_lossfactor <- file.path(dir_meta_mc_outputs, identifier_string, "lossfactor_conspc", paste0("lossfactor_mc", 1:mc_samplesize, ".csv"))
  
  # ensure that all MC lossfactor CSV files exist in the subfolder
  if(mean(map_lgl(filepaths_lossfactor, file.exists)) != 1) stop("Not all expected lossfactor files seem to exist in the specified subfolder")
  ...
  # NOTE: for performance reasons, we use vroom (which outperforms fread from data.table and future_map_dfr) and set all column types to double by hand
  lossfactor_conspc <- vroom::vroom(filepaths_lossfactor, show_col_types = F, col_types = "d") %>%
    
    # # select portfolio, MC run, year and the lossfactor of all relevant countries
    {if(!is.null(iso3_subset)) select(., mc_run, year, all_of(iso3_subset)) else . } %>%
    
    # initiate lazy_dt()
    data.table::as.data.table() %>% dtplyr::lazy_dt() %>%
    
    # add the RCP and SSP
    mutate(rcp = rcp_selected, ssp = ssp_selected, tip = tip_selected, amoc = amoc_selected) %>%
    
    # convert to LONG format (one row per country-year-MC draw pairing)
    pivot_longer(cols = -c(mc_run, year, rcp, ssp, tip, amoc), names_to = "iso3", values_to = "lossfactor_conspc") %>%
  ...
  return(lossfactor_conspc)
  
}

get_climateoutputs <-  function(indicator_selected = "T_AT", # T_AT or SLR
                                mc_samplesize = 10000, # number of MC runs used to calculate the loss distribution
                                rcp_selected = "RCP45", # RCP scenario selected
                                ssp_selected = "SSP2", # SSP scenario selected
                                persist_selected = "1.0", # persistence of temperature impacts selected (1 = none, 0 = 100% persistence)
                                tip_selected = "all", # tipping point modules selected (all/none/individual TPs)
  ...
  # map all expected lossfactor CSV files
  filepaths_climate <- file.path(dir_meta_mc_outputs, identifier_string, indicator_selected, paste0(indicator_selected, "_mc", 1:mc_samplesize, ".csv"))
  
  # ensure that all MC lossfactor CSV files exist in the subfolder
  if(mean(map_lgl(filepaths_climate, file.exists)) != 1) stop("Not all expected climate files seem to exist in the specified subfolder")
  
  df_climate <- vroom::vroom(filepaths_climate, show_col_types = F, col_types = "d")
 
  return(df_climate) 
}

get_lossdistributions <-  function(indicator_selected = "df_npv_bymarket", # df_npv_bymarket or df_npv_fullportfolio
                                    mc_samplesize = 10000, # number of MC runs used to calculate the loss distribution
                                    rcp_selected = "RCP45", # RCP scenario selected
                                    ssp_selected = "SSP2", # SSP scenario selected
                                    persist_selected = "1.0", # persistence of temperature impacts selected (1 = none, 0 = 100% persistence)
                                    ...
                                    base_year = 2024,
                                    end_year = 2100,
                                   additional_suffix = NULL, # can for instance be used to indicate the equity risk premium used
                                    dir_portfolio_simulation = file.path("data", "portfolio_simulation") # directory where loss distributions are stored
) {
  ...
  # add to filename and exit if it does not exist
  filename <- paste0(indicator_selected, "_", identifier_string, "_base", base_year, "_end", end_year, additional_suffix, ".csv")
  if(!file.exists(file.path(dir_portfolio_simulation, indicator_selected, filename))) stop(paste0("Following loss distribution file not found:", filename))

  df <- read_csv(file.path(dir_portfolio_simulation, indicator_selected, filename), show_col_types = F)
  
  return(df) 
}
  
# # test the functions (COMMENTED OUT)
# get_lossfactors(mc_samplesize = mc_samplesize, iso3_subset = c("USA"))
# get_climateoutputs(mc_samplesize = mc_samplesize, indicator_selected = "SLR")
# get_lossdistributions(mc_samplesize = mc_samplesize, indicator_selected = "df_npv_fullportfolio")
...
# NOTE: at 10k MC sample size, this will take several minutes
df_lossfactor_tipnone_rcp45 <- get_lossfactors(mc_samplesize = mc_samplesize,
                                               rcp_selected = "RCP45",
                                               tip_selected = "none",
                                               iso3_subset = "USA")

df_lossfactor_tipall_rcp45 <-  get_lossfactors(mc_samplesize = mc_samplesize,
                                               rcp_selected = "RCP45",
                                               tip_selected = "all",
                                               iso3_subset = "USA")

# read in global mean surface temperature change under RCP4.5 with/without tipping points
df_globaltemp <- bind_rows(get_climateoutputs(mc_samplesize = mc_samplesize, indicator_selected = "T_AT", tip_selected = "none") %>%
                             mutate(tip = "none", rcp = "RCP45"),
                           get_climateoutputs(mc_samplesize = mc_samplesize, indicator_selected = "T_AT", tip_selected = "all", omh_selected = "default", amoc_selected = "IPSL") %>%
                             mutate(tip = "all", rcp = "RCP45"),
                           get_climateoutputs(mc_samplesize = mc_samplesize, indicator_selected = "T_AT", tip_selected = "all") %>%
                             mutate(tip = "all_omhnone", rcp = "RCP45"))

# read in global sea level rise for the same runs
df_globalSLR <- bind_rows(get_climateoutputs(mc_samplesize = mc_samplesize, indicator_selected = "SLR", tip_selected = "none") %>%
                            mutate(tip = "none", rcp = "RCP45"),
                          get_climateoutputs(mc_samplesize = mc_samplesize, indicator_selected = "SLR", tip_selected = "all", omh_selected = "default", amoc_selected = "IPSL") %>%
                            mutate(tip = "all", rcp = "RCP45"),
                          get_climateoutputs(mc_samplesize = mc_samplesize, indicator_selected = "SLR", tip_selected = "all") %>%
                            mutate(tip = "all_omhnone", rcp = "RCP45"))

# repeat for GMST change under RCP8.5
df_globaltemp_rcp85 <- bind_rows(get_climateoutputs(mc_samplesize = mc_samplesize, indicator_selected = "T_AT", rcp_selected = "RCP85", tip_selected = "none") %>%
                                  mutate(tip = "none", rcp = "RCP85"),
                                 get_climateoutputs(mc_samplesize = mc_samplesize, indicator_selected = "T_AT", rcp_selected = "RCP85", tip_selected = "all") %>%
                                  mutate(tip = "all", rcp = "RCP85"))

# repeat for GMST change under RCP2.6
df_globaltemp_rcp26 <- bind_rows(get_climateoutputs(mc_samplesize = mc_samplesize, indicator_selected = "T_AT", rcp_selected = "RCP3-PD26", tip_selected = "none") %>%
                                   mutate(tip = "none", rcp = "RCP3-PD26"),
                                 get_climateoutputs(mc_samplesize = mc_samplesize, indicator_selected = "T_AT", rcp_selected = "RCP3-PD26", tip_selected = "all") %>%
...
mc_runs_screenedout <- c(df_globaltemp %>% filter(is.na(T_AT)) %>% pull(mc_run) %>% unique(),
                         df_globaltemp_rcp85 %>% filter(is.na(T_AT)) %>% pull(mc_run) %>% unique()
) %>% unique()

# check temperature and SLR distribution after removal of NA runs
df_globaltemp %>% filter(!mc_run %in% mc_runs_screenedout) %>% summary()
df_globaltemp %>% filter(!mc_run %in% mc_runs_screenedout) %>% group_by(mc_run) %>%
  summarise(max_T_AT = max(T_AT)) %>% arrange(desc(max_T_AT))
# there is an additional run that produces near-infinite warming

# filter out
mc_runs_screenedout <- c(mc_runs_screenedout,
                         c(df_globaltemp %>% filter(T_AT > 20) %>% pull(mc_run) %>% unique(),
                           df_globaltemp_rcp85 %>% filter(T_AT > 20) %>% pull(mc_run) %>% unique()))

# check temperature and SLR distribution after removal of NA runs
df_globaltemp %>% filter(!mc_run %in% mc_runs_screenedout) %>% summary()
df_globaltemp_rcp85 %>% filter(!mc_run %in% mc_runs_screenedout) %>% summary()
df_globalSLR %>% filter(!mc_run %in% mc_runs_screenedout) %>% summary()

## export mean GMST change and SLR across MC runs under RCP4.5 to facilitate back-of-an-envelope calculations (COMMENTED OUT)
...
#   write_csv(file.path("data", "META MC results", paste0("T_AT_n", mc_samplesize, "_RCP45SSP2_MCmean.csv")))
...
#   
#   write_csv(file.path("data", "META MC results", paste0("SLR_n", mc_samplesize, "_RCP45SSP2_MCmean.csv")))
...
figure1a <- df_globaltemp %>%
  ...
  mutate(label = case_when(tip == "all" ~ label_tip_all,
                           tip == "none" ~ label_tip_none)) %>%
...
df_figure1c <- bind_rows(df_lossfactor_tipnone_rcp45,
  ...
  mutate(lossfactor = 1 - lossfactor_conspc) %>%
  ...
  group_by(year, tip, rcp) %>% 
  summarise_at(vars(lossfactor), list(mean = mean,
...
df_npv_bymarket_example_tipall <- get_lossdistributions(mc_samplesize = mc_samplesize, tip_selected = "all")

# without climate tipping points
df_npv_bymarket_example_tipnone <- get_lossdistributions(mc_samplesize = mc_samplesize, tip_selected = "none")
...
make_figure1d <- function(magnify_factor_climatelosses = 5, magnify_factor_tplosses = 50) {
  ...
  figure1d <- df_pvtimeseries_us %>%
    ...
    left_join(df_figure1c %>% filter(tip %in% c("all", "none")) %>% ungroup() %>%
                select(year, tip, lossfactor_mean = "mean"), by = "year") %>%
    ...
    summarise(currentequity = head(currentequity, 1), # NOTE: current and present values before climate damages are same for tipping point settings, so we just extract the 1st value
              ...
              presentequity_netoflosses_tp_none = presentequity_netoflosses[tip == "none"],
              presentequity_netoflosses_tp_all = presentequity_netoflosses[tip == "all"]
    ) %>%
    ...
    annotate("text", x = 2050, y = 1.05, label = "Climate losses excl.\nclimate tipping points",
             alpha = 1, colour = "#00BFC4", hjust = 0.5, fontface = "bold",
             size = 5/.pt) +
    
    # climate losses w/ TPs
    annotate("text", x = 2080, y = 0.525, label = "Additional losses due\nto tipping points",
...
ggsave(file.path("graphs", paste0(Sys.Date(), " SI_Figure1d_methodology.pdf")),
...
# save out
ggsave(file.path("graphs", paste0(Sys.Date(), " Figure1_methodology.pdf")),
       plot = figure1,
       width = 11.5, height = 9.2, unit = "cm")

# clean up the environment
rm(df_pvtimeseries_us, figure1, figure1a, figure1b, figure1c, figure1d, figure1d_no_rescaling, df_figure1c)
gc()
...
df_npv <- bind_rows(get_lossdistributions(indicator_selected = "df_npv_fullportfolio", mc_samplesize = mc_samplesize,
                                          tip_selected = "all",
                                          rcp_selected = "RCP3-PD26") %>% mutate(persist = as.character(persist)),
                    
                    get_lossdistributions(indicator_selected = "df_npv_fullportfolio", mc_samplesize = mc_samplesize,
                                          tip_selected = "none",
                                          rcp_selected = "RCP3-PD26") %>% mutate(persist = as.character(persist)),
                    
                    get_lossdistributions(indicator_selected = "df_npv_fullportfolio", mc_samplesize = mc_samplesize,
                                          tip_selected = "all",
                                          rcp_selected = "RCP45") %>% mutate(persist = as.character(persist)),
                    ...
                    get_lossdistributions(indicator_selected = "df_npv_fullportfolio", mc_samplesize = mc_samplesize,
                                          tip_selected = "all",
                                          rcp_selected = "RCP85") %>% mutate(persist = as.character(persist)),
...
# save out
ggsave(file.path("graphs", paste0(Sys.Date(), " Figure2_main_results.pdf")),
       plot = figure2, width = 15, height = 10, unit = "cm")

# clean up
rm(figure2, figure2a, figure2b)
gc()
...
# read in the respective PV loss distributions
df_individual_tps <- bind_rows(
  get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "all"),
  get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "none"),
  get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "PCF"),
  get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "GIS"),
  get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "WAIS"),
  get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "AMAZ"),
  get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "ISM")
)
  
# calculate summary statistics, trimming problematic MC runs
df_individual_tps_sumstat <- summarise_npv_distribution(df_individual_tps %>% filter(!mc_run %in% mc_runs_screenedout))
...
ggsave(file.path("graphs", paste0(Sys.Date(), " SI_Figure_Waterfall_EM_FEM.pdf")),
       width = 10, height = 4.5)


#### Figure 4e: Panel on investor horizons

# read in the respective loss distributions (2024-2100, 2034-2110, 2044-2120; each w/ and w/o TPs)
df_timehorizon <- bind_rows(get_lossdistributions("df_npv_fullportfolio",
                                                  mc_samplesize = mc_samplesize,
                                                  base = 2024,
                                                  end = 2100,
                                                  tip_selected = "all"),
                            
                            get_lossdistributions("df_npv_fullportfolio",
                                                  mc_samplesize = mc_samplesize,
                                                  base = 2024,
                                                  end = 2100,
                                                  tip_selected = "none"),
...
df_erp_sumstat <- map_dfr(seq(0.01, 0.15, by = 0.01), ~ bind_rows(get_lossdistributions("df_npv_fullportfolio",
                                                                                       mc_samplesize = mc_samplesize,
                                                                                       tip_selected = "all",
                                                                                       additional_suffix = paste0("_erp", .x %>% str_replace("0.10$", "0.1"))),
                                                                 get_lossdistributions("df_npv_fullportfolio",
                                                                                       mc_samplesize = mc_samplesize,
                                                                                       tip_selected = "none",
                                                                                       additional_suffix = paste0("_erp", .x %>% str_replace("0.10$", "0.1")))
...
figure4 <- ggarrange(# first we combine Panels a and b (= the maps) horizontally as a 1x2
...
# save out
ggsave(file.path("graphs", paste0(Sys.Date(), " Figure4.pdf")),
       plot = figure4, width = 17, height = 12, unit = "cm")
...
# save out
ggsave(file.path("graphs", paste0(Sys.Date(), " SI_Figure4_horizon_erp_EM_FEM.pdf")),
       plot = si_erp_horizon_em_fm,
       width = 12, height = 12, unit = "cm")

# clean up
rm(figure4, panel_map, panel_map_totalloss, panel_waterfall, panel_waterfall_em, panel_waterfall_fm,
   panel_horizon, panel_erp, si_erp_horizon_em_fm)
gc()
...
df_figure3a <- bind_rows(
  ### BHM + uniform persistence 0--1
  get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "all", tdamage_selected = "bhm_distribution", persist_selected = "Distribution") %>%
    mutate(persist = as.character(persist)),
  get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "none", tdamage_selected = "bhm_distribution", persist_selected = "Distribution") %>%
    mutate(persist = as.character(persist)),
  ### BHM + persistence
  get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "all", tdamage_selected = "bhm_distribution", persist_selected = "0.0") %>%
    mutate(persist = as.character(persist)),
  get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "none", tdamage_selected = "bhm_distribution", persist_selected = "0.0") %>%
    mutate(persist = as.character(persist)),
  ### COACCH + no persistence
  get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "all", tdamage_selected = "coacch_distribution", persist_selected = "1.0") %>%
    mutate(persist = as.character(persist)),
  get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "none", tdamage_selected = "coacch_distribution", persist_selected = "1.0") %>%
    mutate(persist = as.character(persist)),
) %>%
  ...
  mutate(tdamage = factor(case_when(tdamage == "bhm_distribution" ~ "Burke et al. (2015)",
                                    tdamage == "coacch_distribution" ~ "Van der Wijst et al. (2023): COACCH"),
...
df_figure3a_summarystat <- df_figure3a %>% summarise_npv_distribution() %>% filter(discountrate == "damodaran")

### calculate the maximum and mimum loss across specifications
# maximum
max_all <- df_figure3a_summarystat %>% filter(portfolio %in% c("msciworld", "msciem", "mscifm")) %>%
  pull(max) %>% max()
# minimum
min_all <- df_figure3a_summarystat %>% filter(portfolio %in% c("msciworld", "msciem", "mscifm")) %>%
  pull(min) %>% min()
...
make_persist_chart <- function(persist_selected = NULL, # persistence selected
  ...
  df_figure3a %>%
    ...
    geom_boxplot(data = df_figure3a_summarystat %>% filter(persist == persist_selected,
                 ...
                 aes(lower = -perc25, upper = -perc75,
                     ...
                     fill = tip),
                 outlier.shape = NA, width = 0.15, colour = "black", linewidth = 0.2, alpha = 0.9,
                 fatten = 1.5) +
    
    # add the expected loss and 95% VaR as text labels
    geom_text(data = df_figure3a_summarystat %>% filter(persist == persist_selected,
                                                        portfolio == portfolio_selected,
                                                        tdamage == tdamage_selected),
              aes(y = -0.48, label = paste0("Mean: ", 100*round(-mean, 3), "%",
...
figure3a <- make_persist_chart(persist_selected = "No persistence", tdamage_selected = "Van der Wijst et al. (2023): COACCH",
...
figure3b <- make_persist_chart(persist_selected = "Uniform distribution of\n 0-100% persistence", tdamage_selected = "Burke et al. (2015)",
                               ylim_selected = c(min_all - 0.1, max_all))

# Figure 3c: MSCI World & Burke et al. (2015), 100% persistence
figure3c <- make_persist_chart(persist_selected = "Full persistence", tdamage_selected = "Burke et al. (2015)",
                               ylim_selected = c(min_all - 0.1, max_all))


# repeat the same for the MSCI EM 
figure3d <- make_persist_chart(persist_selected = "No persistence", tdamage_selected = "Van der Wijst et al. (2023): COACCH", portfolio_selected = "msciem",
                               ylim_selected = c(min_all - 0.1, max_all), min_label_adjustmentfactor = -0.1) +
  theme(strip.text = element_text(size = 5, face = "bold"))

figure3e <- make_persist_chart(persist_selected = "Uniform distribution of\n 0-100% persistence", tdamage_selected = "Burke et al. (2015)", portfolio_selected = "msciem",
                               ylim_selected = c(min_all - 0.1, max_all))
figure3f <- make_persist_chart(persist_selected = "Full persistence", tdamage_selected = "Burke et al. (2015)", portfolio_selected = "msciem",
                               ylim_selected = c(min_all - 0.1, max_all))


# combine the panels into one 2x3 grid chart
figure3 <- ggarrange(figure3a, figure3b, figure3c, figure3d, figure3e, figure3f,
                     ncol = 3, nrow = 2, align = "hv", labels = "auto",
                     font.label = list(size = 7, color = "black", face = "bold"))

# save out
ggsave(file.path("graphs", paste0(Sys.Date(), " Figure3.pdf")),
       plot = figure3, width = 15, height = 10, unit = "cm")

# clean up
rm(figure3, figure3a, figure3b, figure3c, figure3d, figure3e, figure3f)
gc()
...
df_figure5_distr <- bind_rows(get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize,
                                          amoc_selected = "Cai"),
                              get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "all"),
                              get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "none")
) %>% mutate(persist = as.character(persist)) %>%
  ...
  mutate(case = factor(case_when(tip == "none" ~ label_tip_none,
...
figure5a <- df_figure5_distr %>%
  ...
  # add 95% VaR
  geom_point(data = df_figure5_sumstat %>% filter(case != "Excl. climate tipping points"),
             aes(y = perc95, color = case, shape = clean_summarystat_labels("perc95")), alpha = 0.9, size = 1.5) +
  ...
  geom_boxplot(data = df_figure5_sumstat %>% filter(case != "Excl. climate tipping points"),
               ...
               aes(lower = perc25, upper = perc75,
                   middle = median, ymin = perc05, ymax = perc95,
                   fill = case),
               outlier.shape = NA, width = 0.15, colour = "black", linewidth = 0.2,
  ...
  geom_text(data = df_figure5_sumstat %>% filter(case != "Excl. climate tipping points"),
            aes(y = perc99, label = paste0("99% VaR: -", 100*round(perc99, 3), "%")),
            size = 5/.pt, nudge_x = -0.15, hjust = 0) +
...
# RCP4.5 - no AMOC collapse (already loaded above)
df_lossfactor_tipall_rcp45

# RCP4.5 - with AMOC collapse
df_lossfactor_tipall_rcp45_amocCai <- get_lossfactors(mc_samplesize = mc_samplesize, rcp_selected = "RCP45",
                                                      amoc_selected = "Cai", iso3_subset = "USA")

# RCP8.5 - no AMOC collapse
df_lossfactor_tipall_rcp85 <- get_lossfactors(mc_samplesize = mc_samplesize, rcp_selected = "RCP85",
                                              iso3_subset = "USA")

# RCP8.5 - with AMOC collapse
df_lossfactor_tipall_rcp85_amocCai <- get_lossfactors(mc_samplesize = mc_samplesize, rcp_selected = "RCP85",
                                                      amoc_selected = "Cai", iso3_subset = "USA")

# combine the MC percentile runs into one data frame
df_figure5b <- bind_rows(df_lossfactor_tipall_rcp45 %>% 
                           mutate(rcp = "RCP4.5", amoc = figure5_label_tipall_amocnone),
                         
                         df_lossfactor_tipall_rcp45_amocCai %>% 
                           mutate(rcp = "RCP4.5", amoc = figure5_label_tipall_amocCai),
                         
                         df_lossfactor_tipall_rcp85 %>% 
                           mutate(rcp = "RCP8.5", amoc = figure5_label_tipall_amocnone),
                         
                         df_lossfactor_tipall_rcp85_amocCai %>% 
  ...
  group_by(year, rcp, ssp, amoc, iso3) %>%
  summarise(mean = mean(lossfactor_conspc),
            # NOTE: we take lower percentiles here because lossfactor_conspc are reversed (e.g. 0.9 for 10% loss)
            perc95 = quantile(lossfactor_conspc, 0.05),
            perc99 = quantile(lossfactor_conspc, 0.01),
            .groups = "drop") %>%
  ...
  # merge in country names and clean them up 
...
# save out
ggsave(file.path("graphs", paste0(Sys.Date(), " Figure5.pdf")),
       plot = figure5, width = 14, height = 12, unit = "cm")

# clean up
rm(figure5a, figure5b, figure5)
gc()
...
# RCP8.5
df_globalSLR_rcp85 <- bind_rows(get_climateoutputs(mc_samplesize = mc_samplesize, indicator_selected = "SLR", rcp_selected = "RCP85", tip_selected = "none") %>%
                                  mutate(tip = "none", rcp = "RCP85"),
                                get_climateoutputs(mc_samplesize = mc_samplesize, indicator_selected = "SLR", rcp_selected = "RCP85", tip_selected = "all") %>%
                                  mutate(tip = "all", rcp = "RCP85")
)
  
# RCP2.6
df_globalSLR_rcp26 <- bind_rows(get_climateoutputs(mc_samplesize = mc_samplesize, indicator_selected = "SLR", rcp_selected = "RCP3-PD26", tip_selected = "none") %>%
                                  mutate(tip = "none", rcp = "RCP3-PD26"),
                                get_climateoutputs(mc_samplesize = mc_samplesize, indicator_selected = "SLR", rcp_selected = "RCP3-PD26", tip_selected = "all") %>%
                                  mutate(tip = "all", rcp = "RCP3-PD26")
)
...
# NOTE: for 10k MC sample size, this takes several minutes
tempchart_individual_TPs <- c("all", "OMH", "PCF", "AMAZ") %>%
  
  map(.f = function(tip_selected) {
    
    # read in the GMST data for the tipping point specification in question
    get_climateoutputs("T_AT", mc_samplesize = mc_samplesize, tip_selected = tip_selected,
                       omh_selected = ifelse(tip_selected == "OMH", "default", "none")) %>%
      ...
      mutate(facet_label = ifelse(tip_selected == "all", "all_omhnone", tip_selected)) %>%
...
slrchart_individual_TPs <- c("all", "OMH", "PCF", "GIS", "WAIS", "AMAZ") %>%
  
  map(.f = function(tip_selected) {
    
    get_climateoutputs("SLR", mc_samplesize = mc_samplesize, tip_selected = tip_selected,
                       omh_selected = ifelse(tip_selected == "OMH", "default", "none")) %>%
      ...
      summarise_at(vars(SLR), list(mean = mean,
                                   perc025 = ~ quantile(.x, 0.025),
                                   perc975 = ~ quantile(.x, 0.975))) %>%
...
figure_si_countryweights <- df_weights_long %>%
  ...
  # drop all entries with a zero weight
  filter(weight > 0) %>%
...
df_npv_ssp5 <- bind_rows(
  # RCP4.5 - no TPs
  get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "none",
                        ssp_selected = "SSP5", rcp_selected = "RCP45"),
  ...
  # RCP8.5 - no TPs
  get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "none",
                        ssp_selected = "SSP5", rcp_selected = "RCP85"),
...
df_npv_dietz <- bind_rows(
  # RCP4.5 - w/o TPs (which in Dietz et al 2021 also implies that the surface albedo feedback is switched off)
  get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "none", saf_deactivated = T),
  # RCP4.5 - w/ TPs
  get_lossdistributions("df_npv_fullportfolio", mc_samplesize = mc_samplesize, tip_selected = "all", saf_deactivated = F, omh_selected = "default", amoc_selected = "IPSL")
)
  
# calculate summary statistics
df_npv_summarystat_dietz <- df_npv_dietz %>% 
  
  # exclude the problematic runs
  filter(!mc_run %in% mc_runs_screenedout) %>%
...
df_npv_summarystat_dietz %>% filter(portfolio %in% c("msciworld", "msciem", "mscifm"))
