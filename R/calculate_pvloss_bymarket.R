...
#                        ssp_selected = "SSP2",
#                        persist_selected = "1.0",
#                        mc_samplesize = 250,
#                        tip_selected = "all",
#                        tdamage_selected = "coacch_distribution",
#                        omh_selected = "none",
#                        amoc_selected = "none",
#                        saf_deactviated = FALSE,
#                        return_output = FALSE
# )
# base_year = 2024
# end_year = 2100
...
# dir_meta_mc_outputs = "data/META MC results"
# dir_out_npv_bymarket = file.path("data", "portfolio_simulation", "df_npv_bymarket")
# dir_out_npv_fullportfolio = file.path("data", "portfolio_simulation", "df_npv_fullportfolio")
...
calculate_pvloss_bymarket <- function(base_year = 2024, # base year for the dividend NPV
  ...
  if(!is.null(run_identifier_selected)) {
    ...
    saf_deactivated = run_identifier_selected$saf_deactivated
  ...
  if(mean(map_lgl(filepaths_lossfactor, file.exists)) != 1) stop("Not all expectedlossfactor files seem to exist in the specified subfolder")
  
  print("Loading the lossfactor files...")
  ...
  lossfactor_conspc <- vroom::vroom(filepaths_lossfactor, show_col_types = F, col_types = "d") %>%
    ...
    # add the SSP
    mutate(ssp = ssp_selected) %>%
    
    # # select portfolio, MC run, year and the lossfactor of all relevant countries
    # # NOTE: this step is commented out because the Julia scripts already subset to countries with non-zero market cap in the MSCI indices
  ...
  print("Imputing Taiwan w/ Hong Kong")
  ...
  # check for NA patterns if specified by the user
  if(check_na_patterns) {
    
    print("Checking NA patterns...")
  ...
  print("Aggregating lossfactors to market-level losses...")
  ...
  # remove lossfactor_conspc from the environment to save memory
  rm(lossfactor_conspc)
  gc()
  ...
  df_npv_bymarket <- df_npv_bymarket %>%
    ...
    mutate(rcp = rcp_selected,
           ...
           omh = omh_selected,
           amoc = amoc_selected,
           saf_deactivated = saf_deactivated,
           base_year = base_year,
           end_year = end_year)
  
  
  print("Exporting market-level loss distributions...")
  ...
  print("Calculating loss distribution for the entire portfolio...")
  ...
  print("Exporting portfolio-level loss distributions...")
...
# calculate_pvloss_bymarket(mc_samplesize = 250)
...
meta_specs_available <- tibble(identifier_string = file.path("data", "META MC results") %>%
                                 list.files() %>% str_subset("^n")) %>%
  
  # extract all relevant metadata based on the respective filename using regular expressions
  mutate(mc_samplesize = str_extract(identifier_string, "(?<=^n)[:digit:]+(?=_RCP)") %>% as.integer(),
         ...
         amoc_selected = str_extract(identifier_string, "(?<=\\_amoc)[:alpha:]+"),
         saf_deactivated = str_detect(identifier_string, "_safnone$"))

# ## subset to specific combinations of interest if we do not want to re-run the entire batch (COMMENT OUT)
# # a) COACCH runs
...
# otherwise, select the entire batch
meta_specs_selected <- meta_specs_available

# ## remove specifications that have already been calculated to save runtime (COMMENTED OUT - USE THIS IF YOU ADDED SPECIFICATIONS NOT COVERED BY THE 1st BATCH)
# meta_specs_calculated <- file.path("data", "portfolio_simulation", "df_npv_fullportfolio") %>% list.files() %>%
#                             str_extract("(?<=^df_npv_fullportfolio_).+(?=_base.+)")

meta_specs_selected <- meta_specs_selected %>% filter(!identifier_string %in% meta_specs_calculated)
...
for(jj in 1:nrow(meta_specs_selected)) {
  ...
  calculate_pvloss_bymarket(rcp_selected = meta_specs_selected$rcp_selected[jj],
                            ...
                            saf_deactivated = meta_specs_selected$saf_deactivated[jj],
...
### b) runs with alternative time horizons (used in Figure 3)
...
### c) runs with alternative equity risk premiums (used in Figure 3)
...
# ### d) individual selected specifications (COMMENTED OUT, USE THIS TO RUN SPECIFIC COMBINATIONS W/O RE-RUNNING THE ENTIRE BATCH)
# calculate_pvloss_bymarket(mc_samplesize = 250, tip_selected = "OMH", omh_selected = "default")
